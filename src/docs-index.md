# Index des documentations

## Architecture et design patterns

### Architecture

- [Architecture logicielle](./architecture/architecture-logicielle)
  - [OOP vs. Data-First](./architecture/oop-vs-data-first)
- [3-tiers](./architecture/3-tiers)
- [MVC](./architecture/mvc)
- [Micro-services](./architecture/microservices)
- [Clean Architecture](./architecture/clean-architecture)

### Design Patterns

- [Fondamentaux](./design-patterns/fondamentaux)
- [Observer](./design-patterns/observer)
- [Singleton](./design-patterns/singleton)
- [SOLID](./design-patterns/solid)

## Technologies

- [C# / .NET](./technologies/csharp-dotnet/csharp-dotnet)
- [React Native](./technologies/react-native/react-native)
- [Nuxt 4](./technologies/nuxt-4)
- [React / Next](./technologies/react-next/react-next)
- [Tailwind CSS](./technologies/tailwind-css/tailwind-css)

## Déploiement

- [Docker](./deploy/docker)

## Gestion de projet

- [UML](./gestion-projet/uml)

## Git (Versionning)

- [Commits](./git/commits)

## Sécurité

- [Protection utilisateurs (auth, chiffrement...)](./security/protection-utilisateurs)

## Ressources

- [Ressources (à trier)](./resources/a-trier-resources)
